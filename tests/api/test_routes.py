...
def client(mock_luhn_service):
    # Override dependency so FastAPI uses our mock
...
def test_validate_card_invalid(client, mock_luhn_service):
    ...
    mock_luhn_service.validate.return_value = ErrorResponse(
        code="400",
        error_message="Credit card number must be 12-19 digits"
    )

    payload = {"number": "123abc"}
    response = client.post("/validate", json=payload)
    assert response.status_code == 200  # Your endpoint returns ErrorResponse directly

    data = response.json()
    assert data["code"] == "400"
    assert "must be 12-19 digits" in data["error_message"]
